...
class StorageService {
	...
	async saveMediaAsset({
		...
		const metadata: MediaAssetData = {
			...
			videoCodecString: mediaAsset.videoCodecString,
			thumbnailUrl: mediaAsset.thumbnailUrl,
			ephemeral: mediaAsset.ephemeral,
			storageMode: mediaAsset.storageMode,
			sourceHandle: mediaAsset.sourceHandle,
		};
		const storageMode = mediaAsset.storageMode ?? "opfs";
		...
		try {
			...
			try {
				await Promise.all([
					...
					mediaMetadataAdapter.remove(mediaAsset.id),
				]);
			} catch {
	...
	async loadMediaAssetFile({
		...
		if (!metadata) return mediaAssetsAdapter.get(id);
		return this.loadStoredMediaFile({ metadata, mediaAssetsAdapter });
	}
	...
	async loadMediaAsset({
		...
		const file = await this.loadStoredMediaFile({
			metadata,
			mediaAssetsAdapter,
		});
		if (!file) return null;
		...
		return {
			...
			videoCodecString: metadata.videoCodecString,
	...
	private async loadStoredMediaFile({
		metadata,
		mediaAssetsAdapter,
	}: {
		...
		if (metadata.storageMode !== "handle" || !metadata.sourceHandle) {
			return mediaAssetsAdapter.get(metadata.id);
		}

		try {
			const descriptor: FileSystemHandlePermissionDescriptor = { mode: "read" };
			let permission =
				await metadata.sourceHandle.queryPermission?.(descriptor);
			if (
				permission !== undefined &&
				permission !== "granted" &&
				metadata.sourceHandle.requestPermission
			) {
				permission = await metadata.sourceHandle.requestPermission(descriptor);
			}
			if (permission !== undefined && permission !== "granted") return null;
			return await metadata.sourceHandle.getFile();
		} catch (error) {
			if (

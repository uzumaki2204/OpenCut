...
export async function processMediaAssets({
	...
	files: FileList | File[] | MediaFileSource[];
	onProgress?: ({ progress }: { progress: number }) => void;
}): Promise<ProcessedMediaAsset[]> {
	const fileSources = Array.from(
		files as ArrayLike<File | MediaFileSource>,
	).map(
		(source): MediaFileSource =>
			source instanceof File ? { file: source } : source,
	);
	...
	for (const { file, sourceHandle } of fileSources) {
		...
		const storageMode = getMediaStorageMode({ file, sourceHandle });
		if (storageMode === "opfs") {
